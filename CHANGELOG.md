...
- Write the virtual HTML/style/script loader modules at the package's resolved on-disk root, fixing `Package path ./<name>-loader.js is exported from package ... but no valid target file was found` on webpack 5.109+ ([#77](https://github.com/studiometa/playground/pull/77), [d0c924a](https://github.com/studiometa/playground/commit/d0c924a))
- Load `esbuild-wasm` from esm.sh at runtime instead of bundling it, fixing the broken `wasmURL` that stopped the preview from running the compiled script on webpack 5.109+, and keeping the ~11 MB wasm out of consumer bundles ([#78](https://github.com/studiometa/playground/pull/78), [89b55ae](https://github.com/studiometa/playground/commit/89b55ae))
